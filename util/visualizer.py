...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        h, w, _ = im.shape
        if aspect_ratio > 1.0:
            im = Image.fromarray(im)  # Convert to PIL image
            im = im.resize((int(w * aspect_ratio), h), Image.BICUBIC)  # Resize with bicubic interpolation
            im = np.array(im)  # Convert back to NumPy array
        if aspect_ratio < 1.0:
            # Convert to PIL image
            im = Image.fromarray(im)

            # Resize with the specified width and height, using bicubic interpolation
